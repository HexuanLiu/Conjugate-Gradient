...
int main() {
  ...
  loop inner[3] = {
    { "i", "i_begin", "i_end", "i_last", "tile_rows", 1 },
    { "j", "j_begin", "j_end", "j_last", "tile_cols", 1 },
    { "k", "k_begin", "k_end", "k_last", "tile_inner", 2 }
  };

  size_t inner_perm[3] = { 0, 1, 2 };
  ...
  (void) fprintf(source, "    }\n");
  // Done with fringe of innermost loop

  (void) fprintf(source, "  } // %s\n", inner[jp].name.c_str());

  // Fringe of second innermost loop
  (void) fprintf(source, "  for (size_t %s = %s; %s < %s; ++%s) { // %s cleanup \n", 
  ...
  // Fringe innermost
  (void) fprintf(source, "    for (size_t %s = %s; %s < %s; ++%s) {\n",
		 inner[kp].name.c_str(), inner[kp].end.c_str(), inner[kp].name.c_str(), inner[kp].last.c_str(), inner[kp].name.c_str());
  ...
  (void) fprintf(source, "    }\n}\n");
  // End Fringe of second innermost loop
